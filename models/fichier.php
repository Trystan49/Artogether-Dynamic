...
class Fichier
{
        ...
        if (!empty($filesP)) {
            ...
            if (in_array($file_extension, $extension_autorisees) && $file_size <= $size_max) {
                ...
                if (move_uploaded_file($file_tmp_name, $file_dest)) {
                    $req = $pdoP->prepare('INSERT INTO oeuvres (TITRE_OEUVRE, ANNEE_OEUVRE, IMG_OEUVRES, ID_TYPES_OEUVRES) VALUE (?,?,?,?)');
    ...
    public static function download($filesP, $values, $pdoP)
    {
        ...
        $req = $pdoP->prepare('SELECT TITRE_OEUVRE, ANNEE_OEUVRE, IMG_OEUVRES, ID_TYPES_OEUVRES FROM oeuvres');
        $req->execute();
