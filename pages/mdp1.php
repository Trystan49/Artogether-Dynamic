<!-- Dashboard -->
<nav class="navbar navbar-inverse navbar-global navbar-fixed-top">
  <!-- Conteneur du site -->
      <div class="container-fluid">
        <!-- Haut de page -->
        <div class="navbar-header">
          <button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#navbar" aria-expanded="false" aria-controls="navbar">
            <span class="sr-only">Toggle navigation</span>
            <span class="icon-bar"></span>
            <span class="icon-bar"></span>
            <span class="icon-bar"></span>
          </button>
          <!-- Contenu du haut de page-->
          <div class="navbar-title">ARTOGETHER
            <span class="slogan">Apprenons l'art de créer ensemble</span>
            <span class="forgetMdp"><u>L'oubli, c'est la vie !</u> (A.Allais)</span>
            <span class="discover"><em>Bonne découverte !</em></span>
          </div>
        </div>
        <div id="navbar" class="collapse navbar-collapse">
          <p class="navbar-right follow">
            Suivre Artogether 
            <br>
            <a href="https://www.instagram.com/art_tristoune_49/" class="fa fa-instagram"></a>
            <a href="https://www.pinterest.fr/ART_Tristoune/_saved/" class="fa fa-pinterest"></a>
          </p>
        </div>
      </div>
    </nav>

<!-- Contenu -->
<div class="main-content">
  <div class="aide">
    <span class="title">ARTOGETHER</span>
    <br>
    <br>
    <span class="titlebis">Récupération mot de passe</span>
    <form method="get" action="form.php">
      <div class="champ">
          <br>
          <input class="motdepasse" type="text" id="motdepasse" name="pseudo" placeholder="Mot de passe">
      </div>
      <div class="champ">
          <input class="verificationmdp" type="text" id="verificationmdp" name="verificationmdp" placeholder="Vérifier le mot de passe">
      </div>
      <!--bouton d'envoi du formulaire-->
      <div class="champ">
          <input class="send" type="submit" value="Valider">
      </div>
    </form>
  </div>
</div>
